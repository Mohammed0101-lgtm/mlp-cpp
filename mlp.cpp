...
class Data_loader
{
  ...
  public:
    ...
    std::vector<std::stack<std::vector<int>>>
    get_batch(std::vector<int>& data) {
        ...
        for (int i = 0; i < batch_size; i++) {
            ...
            // copy the chunks
            std::vector<int> input_chunck(
                data.begin() + chunck_begin, data.begin() + chunck_end);
            std::vector<int> target_chunck(
                data.begin() + chunck_begin + 1, data.begin() + chunck_end + 1);
...
class Value
{
  ...
  public:
    ...
    Value(const Value& val)
        : data(val.data), grad(val.grad), _prev(val._prev), _op(val._op) {}
    Value(Value& val)
        : data(val.data), grad(val.data), _prev(val._prev), _op(val._op) {}
...
void Value::_backward() {
    if (_op == ' ' || _prev.empty() || !_prev[0]) {
        std::cerr << "Debug : There is a problem in the backward function"
                  << std::endl;
        return;
    } else if (_op == '+') {
        ...
        _prev[0]->grad +=
            exponent * std::pow(_prev[0]->data, exponent - 1) * grad;
    } else if (_op == 'r') {
        // relu'(x) = (x > 0) ? 1 : 0
  ...
  public:
    ...
    class Proxy
    {
      public:
        ...
        grad::Value& operator=(const grad::Value& value) {
    ...
    class ConstProxy
    {
      public:
        ...
        operator const grad::Value&() const {
            return tensor.data[tensor.getIndex(indices)];
        }
    };
    ...
    void set_row(int row_index, const std::vector<grad::Value>& vals) {
        ...
        // Check if the size of the provided values matches the number of
        // columns
        assert(vals.size() == cols);
        // Calculate the starting index in the flattened data vector for the
        // specified row
...
Tensor Tensor::multinomial(int num_samples) const {
    ...
    for (int sample = 0; sample < num_samples; ++sample) {
        ...
        auto  it        = std::lower_bound(
            cumulative_probs.begin(), cumulative_probs.end(), rand_prob);
...
Tensor Tensor::softmax() const {
    ...
    float  maxVal = (*std::max_element(
                        data.begin(), data.end(),
                        [](const grad::Value& a, const grad::Value& b) {
...
// TODO : rebuild this function to process raw matrices

std::vector<std::vector<grad::Value>> matmul(
    const std::vector<std::vector<grad::Value>>& first,
    const std::vector<std::vector<grad::Value>>& second) {
    assert(first[0].size() == second.size());

    Tensor result(static_cast<std::vector<int>>(first.size(), second.shape[1]));
        ...
        for (int j = 0; j < other.shape[1]; j++) {
            ...
            for (int k = 0; k < shape[1]; k++) {
                ...
                sum += data[getIndex(std::vector<int>(a, a + 2))].getData() *
                       other
                           .data[other.getIndex(
                               std::vector<int>(std::vector<int>(_a, _a + 2)))]
                           .getData();
            }
            int a[2] = {i, j};
            result.data[result.getIndex(std::vector<int>(a, a + 2))] =
                grad::Value(sum);
        }
    }
...
class Embedding
{
  public:
    ...
    std::vector<std::vector<grad::Value>> table;

    // Initialize the embedding table with random values
    void _init_(const std::vector<int>& dims) {
        if (dims.size() < 2)
            throw std::runtime_error("Cannot initialize embedding table with "
                                     "fewer than 2 dimensions!");

        table.reserve(dims[0]);
        for (std::vector<grad::Value> row : table)
            row.reserve(dims[1]);
        ...
        for (size_t i = 0; i < dims[0]; i++)
            ...
            for (size_t j = 0; j < dims[1]; j++)
                table[i][j] =
                    grad::Value(static_cast<double>(rand()) / RAND_MAX);
    }

    // Forward pass: lookup embeddings for input indices
    std::vector<std::vector<grad::Value>>
    forward(std::vector<std::vector<grad::Value>>& input) {
        // Assume input is a 1-D tensor of indices
        std::vector<int> indices; // Retrieve indices from the input tensor

        for (std::vector<grad::Value> row : table)
            for (grad::Value val : row)
                indices.push_back(static_cast<int>(val.getData()));

        int d[2] = {
          static_cast<int>(indices.size()), static_cast<int>(table[0].size())};

        // Initialize output tensor with the shape (num_indices, embedding_dim)
        std::vector<std::vector<grad::Value>> output(
            d[0], std::vector<grad::Value>(d[1]));

        // Retrieve embeddings corresponding to each index in the input
        for (size_t i = 0; i < indices.size(); i++) {
            int                      idx = indices[i];
            std::vector<grad::Value> embedding =
                table[idx]; // Get the corresponding row from the table
            output[i] =
                embedding; // Set the retrieved row into the output tensor
        }
    ...
    void backward(
        std::vector<std::vector<grad::Value>>& input,
        std::vector<std::vector<grad::Value>>& grad_output) {
        // Assume input is a 1-D tensor of indices
        std::vector<int> indices; // Retrieve indices from the input tensor
        for (std::vector<grad::Value> row : input)
            for (grad::Value val : row)
                indices.push_back(static_cast<int>(val.getData()));
        ...
        size_t cols =
            table[0].size(); // Number of columns in the embedding table
        for (size_t i = 0; i < indices.size(); i++) {
            int                      idx = indices[i];
            std::vector<grad::Value> grad =
                grad_output[i]; // Get the gradient row corresponding to the
                                // embedding

            // Accumulate the gradient into the corresponding row of the
            // embedding table
            for (size_t j = 0; j < grad.size(); j++) {
                int   table_index = idx * cols + j;
                float g           = table[i][table_index].getGrad();
                g += grad[j].getGrad(); // Update the gradient in the local copy
                                        // of the data
                table[i][table_index].set_grad(g);
            }
        }
    }
};
...
{
  private:
    std::vector<std::vector<grad::Value>> weights;
    float                                 bias = 0.0f;

  public:
    LinearLayer(std::vector<int> dims)
        : weights(std::vector<std::vector<grad::Value>>(
              dims[0], std::vector<grad::Value>(dims[1]))) {}
    LinearLayer(std::vector<std::vector<grad::Value>> wei) : weights(wei) {}
    LinearLayer(std::vector<std::vector<grad::Value>> wei, float b)
        : weights(wei), bias(b) {}
    LinearLayer(size_t size)
        : weights(std::vector<std::vector<grad::Value>>(size)) {
        weights_init_(size);
    }

    int                                   get_size() { return weights.size(); }
    std::vector<std::vector<grad::Value>> get_weights() { return weights; }
    void set_weights(std::vector<std::vector<grad::Value>> wei) {
        weights = wei;
    }
    ...
    void weights_init_(size_t __size) {
        for (int i = 0; i < weights.size(); i++) {
            for (int j = 0; j < weights[i].size(); j++) {
                ...
                weights[i][j] =
                    (static_cast<float>(rand()) / (float)RAND_MAX * 2 - 1);
            }
        }
    }

    std::vector<std::vector<grad::Value>>
    feed_forward(std::vector<std::vector<grad::Value>>& data) {
        if (data.size() != weights.size())
            throw std::runtime_error(
                "Weights size should be equal to input size");

        std::vector<std::vector<grad::Value>> output = matmul(weights);
        output                                       = output.relu();
  ...
  public:
    NeuralNetwork(size_t input_size)
        ...
        for (size_t i = 0; i < input_size * 2 + 1; i++) {
            ...
            lookup_table._init_(
                std::vector<int>(table_dimensions, table_dimensions + 2));
        }
    }
    ...
    void           backpropagate(
                  tensor::Tensor& output, tensor::Tensor& targets,
                  float learning_rate = 0.01);

    tensor::Tensor generate(tensor::Tensor idx, size_t max_tokens);

    float
    mse_loss(const tensor::Tensor& predicted, const tensor::Tensor& targets);
};

// mean squared error loss (for simplicity)
float NeuralNetwork::mse_loss(
    const tensor::Tensor& predicted, const tensor::Tensor& targets) {
...
void NeuralNetwork::backpropagate(
    tensor::Tensor& output, tensor::Tensor& targets, float learning_rate) {
    ...
    std::vector<grad::Value> output_layer_weights_vals =
        output_layer_weights.get_data();

    for (size_t i = 0; i < output_layer_weights_vals.size(); i++) {
        float grad = d_logits.get_data()[i].getGrad();
        output_layer_weights_vals[i] =
            output_layer_weights_vals[i] - (learning_rate * grad);
    ...
    std::vector<grad::Value> hidden_layer_weights_vals =
        hidden_layer_weights.get_data();

    for (size_t i = 0; i < hidden_layer_weights_vals.size(); i++) {
        float grad = hidden_grad.get_data()[i].getGrad();
        hidden_layer_weights_vals[i] =
            hidden_layer_weights_vals[i] - (learning_rate * grad);
    ...
    tensor::Tensor lookup_grad = lookup_table.forward(
        hidden_grad); // Assume this propagates gradients properly
    ...
    for (size_t i = 0; i < lookup_grad_vals.size(); i++) {
        ...
        lookup_table_vals[index] =
            lookup_table_vals[index] - (learning_rate * grad);
        lookup_table_vals[index].set_grad(
            0.0f); // Reset the gradient after update
    }
...
std::unordered_map<std::string, float> estimate_loss(
    nn::NeuralNetwork& model, std::vector<int> train_data,
    ...
    for (const std::string& split : splits) {
        ...
        for (int k = 0; k < eval_iters; ++k) {
            std::vector<std::stack<std::vector<int>>> batches =
                loader.get_batch(split == "train" ? train_data : val_data);
        ...
        float mean_loss =
            std::accumulate(losses.begin(), losses.end(), 0.0f) / eval_iters;
        out[split] = mean_loss;
    }
...
int main(void) {
    Data_loader loader;
    std::string input = loader.read_file("input_.txt");
    if (input.empty()) {
