...
class Value {
  public:
    ...
    Value(Value* val) : data(val->data), grad(val->grad), _prev(val->_prev), _op(val->_op) {}
    ...
    Value(Value&& other) noexcept
        : data(other.data), grad(other.grad), _prev(std::move(other._prev)), _op(other._op),
          index(other.index) {
  ...
  private:
    ...
    void build_topo(
        std::list<std::shared_ptr<Value>>& topo, std::set<std::shared_ptr<Value>>& visited,
...
std::vector<std::vector<data::Value>> matmul(
    const std::vector<std::vector<data::Value>>& A,
...
class Embedding {
  public:
    ...
    Embedding(const std::vector<int>& dims) {
        if (dims.size() < 2)
            throw std::runtime_error("Cannot initialize embedding table with fewer than 2 dimensions!");
    ...
    std::vector<std::vector<data::Value>> forward(const std::vector<int>& input_indices) {
  ...
  // Backward pass: update embedding table gradients
    void backward(
        const std::vector<int>& input_indices, // Input should be indices
        ...
        for (size_t i = 0; i < input_indices.size(); i++) {
            ...
            const std::vector<data::Value>& grad = grad_output[i];  // Get the gradient row corresponding to the embedding
...
class LinearLayer {
  ...
  public:
    LinearLayer(std::vector<int> dims)
        : weights(
              std::vector<std::vector<data::Value>>(dims[0], std::vector<data::Value>(dims[1]))) {}
    LinearLayer(std::vector<std::vector<data::Value>> wei) : weights(wei) {}
    LinearLayer(std::vector<std::vector<data::Value>> wei, float b) : weights(wei), bias(b) {}
    LinearLayer(size_t size) : weights(std::vector<std::vector<data::Value>>(size)) {
        weights_init_(size);
    }
...
class NeuralNetwork {
  ...
  public:
    NeuralNetwork(size_t input_size) : hidden_layer(input_size), output_layer(1) {
    ...
    std::vector<data::Value> feed_forward(std::vector<std::vector<data::Value>>& input_data) {
        size_t input_size = input_data.size();
        // look up the tokens indexing
        std::vector<std::vector<data::Value>> logits = lookup_table.forward(input_data);
    ...
    void backpropagate(
        std::vector<data::Value>& output, std::vector<data::Value>& targets, float learning_rate) {
        ...
        std::vector<std::vector<data::Value>> lookup_grad =
            lookup_table.forward(hidden_grad); // Assume this propagates gradients properly
    ...
    std::vector<int> generate(std::vector<data::Value>& idx, size_t max_tokens) {
        std::vector<int> cat;
        // generate the desired max tokens
        for (size_t i = 0; i < max_tokens; i++) {
            // compute the logits
            // TODO : convert 'idx' to a matrix of the weights dimensions
            std::vector<std::vector<data::Value>> mat =
                data::vectorToMatrix(idx, batch_size, context_length);
            std::vector<data::Value> logits = feed_forward(mat);
    ...
    float
    mse_loss(const std::vector<data::Value>& predicted, const std::vector<data::Value>& targets) {
        // initialize
...
estimate_loss(nn::NeuralNetwork& model, std::vector<int> train_data, std::vector<int> val_data) {
    ...
    for (const std::string& split : {"train", "val"}) {
        ...
        for (int k = 0; k < eval_iters; k++) {
            ...
            std::vector<std::vector<data::Value>> converted_input;
            for (std::vector<int> row : inputs) {
                std::vector<data::Value> r;
                for (int val : row)
                    r.push_back(data::Value(static_cast<float>(val)));

                converted_input.push_back(r);
            }
            ...
            std::vector<data::Value> logits = model.feed_forward(converted_input);
