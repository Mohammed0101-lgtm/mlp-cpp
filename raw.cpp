...
// helpers
template<typename T>
void append_matrix(std::vector<std::vector<T>>& _mat, const std::vector<std::vector<T>>& _to_add) {
    size_t __size = _mat.size() + _to_add.size();
    _mat.resize(__size);

    for (unsigned int k = 0, i = _mat.size(); i < __size; i++, k++)
        _mat[i] = _to_add[k];
}

template<typename T>
void append_vector(std::vector<T>& _vec, const std::vector<T>& _to_add) {
    size_t __size = _vec.size() + _to_add.size();

    for (unsigned int k = 0, i = _vec.size(); i < __size; i++, k++)
        _vec[i] = _to_add[k];
}
...
class Data_loader
{
   ...
   public:
    ...
    std::tuple<std::vector<std::vector<int>>, std::vector<int>> get_batch(std::vector<int>& _data) {
        // generate a random index for the offset where the batch begins
        int rand_offset = rand() % (_data.size() - (CONTEXT_LENGTH) *BATCH_SIZE);
        ...
        for (int i = 0; i < BATCH_SIZE; i++)
        {
            ...
            // copy the chunks
            std::vector<int> input_chunck(_data.begin() + chunck_begin, _data.begin() + chunck_end);
            // add them to the stack
            inputs.push_back(input_chunck);
            targets.push_back(_data[chunck_end + 1]);
    ...
    std::vector<int> encode(const std::string& _str) {
    ...
    // decode string
    std::string decode(const std::vector<int>& _tokens) {
        std::string result;
        for (int n : _tokens)
...
class Value
{
   public:
    ...
    Value(float _d, const std::vector<std::shared_ptr<Value>>& _children, char _op) :
        data(_d),
        grad(0.0f),
        _prev(_children),
        _op(_op) {}
    ...
    Value& operator=(const Value& _other) {
        ...
        {  // self-assignment check
            this->data  = static_cast<float>(_other.data);
            this->grad  = static_cast<float>(_other.grad);
            this->_prev = static_cast<std::vector<std::shared_ptr<Value>>>(_other._prev);
            this->_op   = static_cast<char>(_other._op);
            this->index = static_cast<int>(_other.index);
        }
    ...
    Value& operator=(Value&& _other) noexcept {
        if (this != &_other)
        {
            // Handle assignment
            if (_other.data != 0.0f || _other.grad != 0.0f || _other._op != ' ')
            {
                data  = _other.data;
                grad  = _other.grad;
                _prev = std::move(_other._prev);
    ...
    Value operator+(const Value& _other) const {
        ...
        children.push_back(std::make_shared<Value>(_other));
        Value res((data + _other.data), children, '+');
    ...
    // Value(this) * scalar
    Value operator*(int _scalar) const {
        std::vector<std::shared_ptr<Value>> children;
        children.push_back(std::make_shared<Value>(*this));
        Value res((data * _scalar), children, '*');
        return res;
    }

    // this / Value(other)
    float operator/(const Value& _other) const { return data / _other.data; }

    // Value(this) ** Value(other)
    Value _pow(const Value& _other) {
        // using smart pointer is just simply smarter
        std::vector<std::shared_ptr<Value>> children;
        children.push_back(std::make_shared<Value>(*this));
        data = pow(data, _other.data);
   ...
   private:
    ...
    void build_topo(std::list<std::shared_ptr<Value>>& _topo,
                    std::set<std::shared_ptr<Value>>&  _visited,
                    std::shared_ptr<Value>             _node) {

        if (_visited.find(_node) == _visited.end())
        {
            _visited.insert(_node);
            for (auto child : _node->_prev)
                build_topo(_topo, _visited, child);

            _topo.push_back(_node);
        }
    }
};
...
std::vector<Value> softmax(std::vector<Value>& data) {
    std::vector<Value> result;
    float              maxVal =
      (*std::max_element(data.begin(), data.end(), [](const Value& a, const Value& b) { return a.data < b.data; }))
        .data;
...
class Embedding
{
   public:
    ...
    std::vector<std::vector<data::Value>> table_forward(const std::vector<int>& input_indices) const {
        ...
        std::vector<std::vector<data::Value>> output(num_indices, std::vector<data::Value>(embedding_dim));
    ...
    void backward(const std::vector<int>&                      input_indices,  // Input should be indices
        ...
        for (size_t i = 0; i < input_indices.size(); i++)
        {
            ...
            if (idx >= table.size() || idx < 0)
                throw std::out_of_range("Index out of range in embedding table during backward pass.");
            ...
            for (size_t j = 0; j < grad.size(); j++)
                embedding[j].grad += grad[j].grad;  // Update gradient for each element in the embedding
        }
    }
};
...
class NeuralNetwork
{
   ...
   public:
    ...
    void backpropagate(std::vector<data::Value>& output, std::vector<data::Value>& targets, float learning_rate) {
        ...
        // dimensions
        std::vector<std::vector<data::Value>> mat         = data::vectorToMatrix(output, BATCH_SIZE, CONTEXT_LENGTH);
        ...
        for (size_t i = 0; i < hidden_layer_weights.size(); i++)
        {
            ...
            {
                hidden_layer_weights[i][j].data -= (learning_rate * hidden_layer_weights[i][j].grad);
...
void train_model(nn::NeuralNetwork& _model, std::vector<int>& _data) {
    Data_loader      loader;
    size_t           data_size  = _data.size();
    size_t           train_size = static_cast<size_t>(data_size * 0.9);
    std::vector<int> train_data(_data.begin(), _data.begin() + train_size);
    std::vector<int> val_data(_data.begin() + train_size, _data.end());

    for (int iter = 0; iter < MAX_ITERATIONS; iter++)
    {
        if (iter % EVAL_INTERVAL == 0)
        {
            auto losses = estimate_loss(_model, train_data, val_data);
            std::cout << "step " << iter << ": train_loss " << losses["train"] << ", val loss" << losses["val"]
                      << std::endl;
        }
        ...
        std::vector<data::Value> logits;
        for (unsigned int i = 0; i < BATCH_SIZE; i++)
            append_vector<data::Value>(logits, _model.feed_forward(inputs[i]));
        ...
        _model.backpropagate(logits, converted_targets, 0.01f);
    }
}
