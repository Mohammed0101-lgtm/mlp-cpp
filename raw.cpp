...
namespace data
{
    ...
    public:
        ...
        Value(float d) : data(d),
                         grad(0.0f),
                         _op(' ') {}

        Value(float d, const std::vector<std::shared_ptr<Value>> &children, char op) : data(d),
                                                                                       grad(0.0f),
                                                                                       _prev(children),
                                                                                       _op(op) {}

        Value(Value &&other) noexcept : data(other.data),
                                        grad(other.grad),
...
namespace nn
{
    ...
    public:
        ...
        LinearLayer(std::vector<int> dims) : weights(std::vector<std::vector<data::Value>>(dims[0], std::vector<data::Value>(dims[1])))
        {
        }

        LinearLayer(std::vector<std::vector<data::Value>> wei) : weights(wei) {}

        LinearLayer(std::vector<std::vector<data::Value>> wei, float b) : weights(wei),
                                                                          bias(b) {}
    ...
    public:
        NeuralNetwork(size_t input_size, int vocab_size) : hidden_layer(input_size),
                                                           output_layer(1)
        {
