...
public:
    ...
    std::tuple<torch::Tensor, torch::Tensor> get_batch(const torch::Tensor& data) {
        ...
        size_t rand_offset = static_cast<size_t>(std::rand()) % (data_size - context_length * batch_size);
...
public:
    ...
    : lookup_table(register_module("lookup_table", torch::nn::Embedding(vocab_size, embd_dim))),
      hidden_layer(register_module("hidden_layer", torch::nn::Linear(context_length * embd_dim, layer_size))),
    ...
    torch::Tensor feed_forward(torch::Tensor input) {
        ...
        if (input.size(1) != context_length * embd_dim) {
            std::cerr << "Error: Mismatch in input dimension after embedding. "
                      << "Expected " << context_length * embd_dim << ", got " << input.size(1) << std::endl;
    ...
    std::string generate(DataLoader& data_loader, const std::vector<int>& context, int max_length = 50) {
        ...
        if (input.size(1) < context_length) {
            std::cerr << "Error: Initial context length is smaller than context_length." << std::endl;
            return "";
        }
...
int main() {
    try {
        DataLoader data_loader;
        std::string input_data = data_loader.read_file("/Users/mac/lmc/input.txt");
