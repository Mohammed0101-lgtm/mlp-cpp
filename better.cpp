...
class DataLoader
{
  ...
  public:
    ...
    get_batch(const torch::Tensor& data) {
        torch::Tensor inputs =
            torch::empty({batch_size, context_length}, torch::kInt32);
        torch::Tensor targets   = torch::empty({batch_size}, torch::kInt64);
        size_t        data_size = data.size(0);

        if (data_size <= context_length) {
            throw std::runtime_error(
                "Data size must be greater than context length");
        }

        size_t rand_offset = static_cast<size_t>(std::rand()) %
                             (data_size - context_length * batch_size);
    ...
    split_data(const std::vector<int>& data) {
...
class LanguageModel : public torch::nn::Module
{
  public:
    ...
    LanguageModel(int vocab_size)
        : lookup_table(register_module(
              "lookup_table", torch::nn::Embedding(vocab_size, embd_dim))),
          hidden_layer(register_module(
              "hidden_layer",
              torch::nn::Linear(context_length * embd_dim, layer_size))),
          output_layer(register_module(
              "output_layer", torch::nn::Linear(layer_size, vocab_size))) {}
    ...
    torch::Tensor feed_forward(torch::Tensor input) {
        ...
        if (input.size(1) != context_length * embd_dim) {
            std::cerr << "Error: Mismatch in input dimension after embedding. "
                      << "Expected " << context_length * embd_dim << ", got "
    ...
    std::unordered_map<std::string, float> estimate_loss(
        DataLoader& data_loader, torch::Tensor train_data,
        ...
        for (const std::string& split : {"train", "val"}) {
            ...
            for (int k = 0; k < eval_iters; k++) {
                ...
                torch::Tensor loss =
                    torch::nn::functional::cross_entropy(predicted, targets);
    ...
    void train_model(DataLoader& data_loader, const std::vector<int>& data) {
        ...
        torch::optim::AdamW optimizer(
            parameters(), torch::optim::AdamWOptions(0.001));

        for (int iter = 0; iter < max_iterations; iter++) {
            if (iter % eval_interval == 0) {
                auto losses =
                    estimate_loss(data_loader, train_tensor, val_tensor);
                std::cout << "Step " << iter
    ...
    std::string generate(
        DataLoader& data_loader, const std::vector<int>& context,
        int max_length = 50) {
        std::vector<int> generated = context;
        torch::Tensor    input =
            torch::tensor(generated, torch::kInt32).unsqueeze(0);
        ...
        for (int i = 0; i < max_length; ++i) {
            if (input.size(1) > context_length) {
                input = input.slice(
                    1, input.size(1) - context_length, input.size(1));
            }
...
int main() {
    try {
        ...
        std::string input_data =
            data_loader.read_file("/Users/mac/lmc/input.txt");
