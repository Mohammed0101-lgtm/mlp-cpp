...
tensor::Tensor NeuralNetwork::feed_forward(tensor::Tensor& input_data) {
    ...
    tensor::Tensor logits = input_layer.feed_forward(input_data);
...
void NeuralNetwork::backpropagate(tensor::Tensor& output, tensor::Tensor& targets, float learning_rate = 0.01) {
    ...
    tensor::Tensor input_grad = input_layer.feed_forward(hidden_grad); 
    tensor::Tensor input_layer_weights = input_layer.get_weights();
    std::vector<grad::Value> input_layer_weights_vals = input_layer_weights.get_data();
    ...
    tensor::Tensor updated_input_weights(input_layer_weights_vals);
